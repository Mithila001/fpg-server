...
def add_adjacency_constraint(model: cp_model.CpModel, room1: Room, room2: Room) -> Dict[str, Any]:
    """Ensures room1 and room2 are touching on one side."""
    
    # 1. Create boolean variables for each side
    ...
    # 3. Logic: At least ONE side must touch
    model.AddBoolOr([touch_right, touch_left, touch_top, touch_bottom])

    # 4. Overlap to ensure they don't just touch at corners
    MIN_WALL = 10 
