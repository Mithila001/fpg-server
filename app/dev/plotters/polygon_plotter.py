...
class PolygonPlotter:
    ...
    def __init__(
        ...
        floor_output_dir_name: str = "floor_saved_images",
    ):
        """
        ...
        Args:
            ...
            floor_output_dir_name: Directory name for saved floor-plan images
                (used by :meth:`floor_plan_plot`).
        """
        self.output_base_dir = output_base_dir
        self.single_output_dir_name = single_output_dir_name
        self.floor_output_dir_name = floor_output_dir_name
    ...
    # convenience alias for boundary-like shapes
    def plot_boundary(
        self,
        coordinates_list: Sequence[Sequence[Coordinate]],
        output_path: Optional[str] = None,
        show: bool = False,
        title: str = "Boundary Visualization",
        figsize: Tuple[int, int] = (8, 8),
    ) -> Tuple[Figure, Axes]:
        """
        Plot a polygon set intended as a boundary or usable‑space result.

        This is simply a thin wrapper around :meth:`polygon_line_plotter_single`
        provided for semantic clarity in calling code.
        """
        return self.polygon_line_plotter_single(
    ...
    def floor_plan_plot(
        ...
        """
        Plot a floor‑plan arrangement of rectangular rooms using solver/room objects.

        Rooms are expected to have attributes ``x``, ``y``, ``w``, ``h`` and
        ``name``; the solver must implement ``Value(var)`` to retrieve numeric
        solutions (as for OR‑tools).

        Args:
            rooms_list: Iterable of room objects.
            solver: Solver instance with ``Value`` method.
            LAND_WIDTH: Width of the containing area.
            LAND_HEIGHT: Height of the containing area.
            output_path: Optional file path for the PNG output.
            show: Whether to call ``plt.show()``.
            title: Plot title.
            figsize: Matplotlib figure size.
            batch_no: Optional batch identifier used when auto‑naming files.
        ...
        # handle output path
        if output_path is None:
            floor_dir = os.path.join(self.output_base_dir, self.floor_output_dir_name)
            os.makedirs(floor_dir, exist_ok=True)
            filename = self._file_name_generator(batch_no or 0, 1)
            output_path = os.path.join(floor_dir, filename)
