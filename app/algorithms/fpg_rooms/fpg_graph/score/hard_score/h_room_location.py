...
def evaluate_room_location_hard(
    ...
    results["veranda_in_bottom_row"] = bool(veranda_nodes) and all(
        compute_zone(node.x, node.y, min_x, min_y, width, height)[1] == 1
        for node in veranda_nodes
    )
    ...
    results["kitchen_not_in_center"] = bool(kitchen_nodes) and all(
        compute_zone(node.x, node.y, min_x, min_y, width, height) != (2, 2)
