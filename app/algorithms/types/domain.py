...
class ConfigData:
    ...
    envelope_exclude_types: list[str] = field(
        default_factory=lambda: ENVELOPE_EXCLUDE_TYPES.copy()
    )
    ...
    constraint_soft_room_adjacency_preference: bool = (
        CONSTRAINT_SOFT_ROOM_ADJACENCY_PREFERENCE
    )
    constraint_soft_compact_layout_center_proximity: bool = (
        CONSTRAINT_SOFT_COMPACT_LAYOUT_CENTER_PROXIMITY
    )
    constraint_soft_bathroom_location_preference: bool = (
        CONSTRAINT_SOFT_BATHROOM_LOCATION_PREFERENCE
    )
    constraint_soft_layout_dead_space_penalty: bool = (
        CONSTRAINT_SOFT_LAYOUT_DEAD_SPACE_PENALTY
    )
    constraint_soft_seed_facade_depth_penalty: bool = (
        CONSTRAINT_SOFT_SEED_FACADE_DEPTH_PENALTY
    )
    constraint_soft_seed_facade_alignment_penalty: bool = (
        CONSTRAINT_SOFT_SEED_FACADE_ALIGNMENT_PENALTY
    )
    constraint_soft_recessed_facade_penalty: bool = (
        CONSTRAINT_SOFT_RECESSED_FACADE_PENALTY
    )
