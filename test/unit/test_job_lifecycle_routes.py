...
class _StubRegistry:
    def __init__(self):
        ...
        self.job_response = self.cancel_response["job"]
...
def test_submit_endpoints_and_lockout(monkeypatch):
    ...
    response = client.post("/algorithms/format/v2", json=format_payload, headers={"X-Client-Id": "cli-1"})
    assert response.status_code == 202
    assert response.json()["job_id"] == "job-1"

    buildable_payload = {
        "area": 1500000,
        "segmentsCoordinates": [{"x": 0, "y": 0}, {"x": 1000, "y": 0}, {"x": 1000, "y": 1000}],
        "roadConnected": [],
    }
    response = client.post("/algorithms/buildable-space", json=buildable_payload, headers={"X-Client-Id": "cli-1"})
    assert response.status_code == 202

    stub.submit_response = {"accepted": False, "message": "Process is Already Running", "job_id": "job-1"}
    lockout = client.post("/algorithms/format/v2", json=format_payload, headers={"X-Client-Id": "cli-1"})
...
def test_cancel_and_status_endpoints(monkeypatch):
    ...
    cancel = client.post("/algorithms/cancel", json={"job_id": "job-1"}, headers={"X-Client-Id": "cli-1"})
