...
def add_minimum_area_coverage(model, all_vars, land_width, land_height, min_coverage=0.8):
    """
    ...
    Args:
        ...
        land_width: Total land width
        land_height: Total land height
        min_coverage: Minimum coverage ratio (default 0.8 for 80%)
    """
    total_land_area = land_width * land_height
    min_required_area = int(total_land_area * min_coverage)
    ...
    for room_name, vars_dict in all_vars.items():
        ...
        area_var = model.NewIntVar(0, land_width * land_height, f'{room_name}_area')
    ...
    print(f"Added minimum area constraint: {min_required_area}/{total_land_area} ({min_coverage*100}%)")
